...
pub enum ScrapComplete {
    DriveErro,
    ScrapComplete,

}
pub async fn scrap(product : &str) -> Result<ScrapComplete, Box<dyn Error + Send + Sync>>{
    ...
    let driver = match WebDriver::new("http://localhost:4444", caps).await{
        Ok(drive) => drive,
        Err(_) => {
            return Ok(ScrapComplete::DriveErro);
        }
    };
    ...
    let mut file  = File::create(format_product_json(product)).await?;
    let json = serde_json::to_string_pretty(&vec_products)?;
    file.write_all(json.as_bytes()).await?;

    // Fechando o navegador
    driver.quit().await?;
    Ok(ScrapComplete::ScrapComplete)
}

fn format_product_json(product : &str) -> String{
    format!("{}.json", product)
}
